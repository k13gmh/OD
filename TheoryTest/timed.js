...
async function init() {
    ...
    // UPDATED: Check localStorage for token [cite: 2026-01-11, 2026-01-17]
    if (!localStorage.getItem('orion_session_token')) {
        document.getElementById('q-text').innerText = "Security token missing. Redirecting...";
    ...
    try {
        ...
        // Anti-Cheat: Clear previous progress but KEEP the security token [cite: 2026-01-11]
...
function startFreshSession() {
    ...
    let selected = [...allQuestions].sort(() => 0.5 - Math.random()).slice(0, 50);
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        totalSeconds--;
        if (totalSeconds < 0) { stopTimerInternal(); forceFinish(); return; }
...
function stopTimerInternal() { if (timerInterval) clearInterval(timerInterval); }
...
function showSummary() {
    ...
    summaryUI.innerHTML = `<h3>Session Summary</h3><p>You have ${skipped} unanswered questions.</p><div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;"><button class="btn btn-blue" onclick="retrySubset('skipped')">SKIPPED (${skipped})</button><button class="btn btn-blue" onclick="retrySubset('flagged')">FLAGGED (${flagged})</button><button class="btn btn-blue" onclick="retrySubset('all')">REVIEW ALL</button><button class="btn" style="background:#2ecc71; color:white;" onclick="finalSubmission()">FINISHED</button></div>`;
}
...
function forceFinish() { stopTimerInternal(); alert("TIME EXPIRED!"); finalSubmission(); }

function finalSubmission() {
    stopTimerInternal();
    let score = 0;
    originalSessionQuestions.forEach(q => { if (testData.selections[q.id] === q.correct) score++; });
...
function saveProgress() { 
    localStorage.setItem('orion_current_session', JSON.stringify({ questions: originalSessionQuestions, data: testData, currentIndex })); 
}
